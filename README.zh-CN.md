...
- `lsp-bridge-elixir-lsp-server`: Elixir 语言的服务器， 可以选择`dexter`（默认）, `elixirLS`, `lexical`或者`nextls`
- `lsp-bridge-python-lsp-server`: Python 语言的服务器， 可以选择 `basedpyright`, `pyright`, `pyright-background-analysis`, `jedi`, `python-ms`, `pylsp`, `ruff`, `ty`, 需要注意的是, `lsp-bridge-multi-lang-server-mode-list` 的优先级高于 `lsp-bridge-single-lang-server-mode-list`, 如果你只想使用单服务器， 请先去掉 `lsp-bridge-multi-lang-server-mode-list` 中 python-mode 的设置
...
| C#          | [csharp-ls](https://github.com/razzmatazz/csharp-language-server)                                   | 使用命令 `dotnet tool install -g csharp-ls` 安装 csharp-ls， `lsp-bridge-csharp-lsp-server` 设置成 `csharp-ls`                                                                                                              |
|             | [omnisharp-dotnet](https://github.com/OmniSharp/omnisharp-roslyn)                                  | OmniSharp 是 .NET 开发平台, 使用命令 `M-x lsp-bridge-install-omnisharp` 来安 OmniSharp, 默认是 `omnisharp-mono`. `lsp-bridge-csharp-lsp-server` 设置成 `omnisharp-dotnet`                                                     |
...
| Deno        | [deno](https://deno.land)                                                                          | Deno 使用 TypeScript 来编程， 你需要定制选项 `lsp-bridge-get-single-lang-server-by-project` 当工程是 Deno 项目的路径时， 返回 "deno" 字符串。也可以在 `lsp-bridge-typescript-lsp-server` 和 `lsp-bridge-typescript-react-lsp-server` 中设置。                                                                                   |
| Dockerfiles | [docker-language-server](https://github.com/rcjsuen/dockerfile-language-server-nodejs)             |                                                                                                                                                                                                                               |
| Elixir      | [dexter](https://github.com/dexter-elixir/dexter)                                                  |                                                                                                                                                                                                                               |
|             | [elixirLS](https://github.com/elixir-lsp/elixir-ls)                                                | 请确保导出 `elixir-ls` 目录到你系统的 PATH 路径                                                                                                                                                                               |
...
|             | [marksman](https://github.com/artempyanykh/marksman)                                               | `lsp-bridge-markdown-lsp-server` 设置成 `marksman`                                                                                                                                                                          |
...
|             | [pyright](https://github.com/microsoft/pyright)                                                    | `pip install pyright`, `lsp-bridge-python-lsp-server` 设置成 `pyright`                                                                                                                                                       |
...
|             | [air](https://github.com/posit-dev/air)                                                            | `lsp-bridge-r-lsp-server` 设置成 `air`， 或者使用 `rlanguageserver_air` 组合配置                                                                                                                                             |
...
| Slint       | [slint-lsp](https://docs.slint.dev/latest/docs/slint/guide/tooling/manual-setup/)                 | `cargo install slint-lsp`，或者从最新的 Slint release 下载预编译二进制文件                                                                                                                                                    |
...
| 路径                                | 作用                                                                                                                 |
|:------------------------------------|:---------------------------------------------------------------------------------------------------------------------|
| lsp-bridge.el                       | Elisp 主入口， 包含用户选项、 mode 集成以及调用 Python 后端的 RPC 入口                                               |
| lsp-bridge-epc.el                   | Elisp 和 Python 子进程之间的 EPC 通讯层                                                                              |
| lsp-bridge-breadcrumb.el            | 当前符号和位置的 breadcrumb 界面集成                                                                                 |
| lsp-bridge-imenu.el                 | 基于 LSP document symbols 的 Imenu 支持                                                                              |
| lsp-bridge-call-hierarchy.el        | 调用层级界面                                                                                                         |
| lsp-bridge-code-action.el           | Code Action 命令和界面                                                                                                |
| lsp-bridge-diagnostic.el            | 诊断信息的渲染、跳转和相关命令                                                                                       |
| lsp-bridge-ref.el                   | 引用查看、批量重命名流程以及相关过滤逻辑                                                                             |
| lsp-bridge-peek.el                  | 用 peek window 查看定义和引用                                                                                        |
| lsp-bridge-inlay-hint.el            | Inlay Hint 渲染                                                                                                      |
| lsp-bridge-semantic-tokens.el       | Semantic Tokens 语义高亮                                                                                             |
| lsp-bridge-jdtls.el                 | Java 相关辅助能力， 例如第三方库跳转                                                                                 |
| lsp-bridge-dart.el                  | Dart 私有协议支持                                                                                                    |
| lsp-bridge-org-babel.el             | Org Babel 代码块的 LSP 支持                                                                                          |
| lsp-bridge-rust.el                  | Rust 相关集成辅助                                                                                                    |
| lsp-bridge-lsp-installer.el         | TabNine 和 OmniSharp 的安装辅助                                                                                      |
| lsp_bridge.py                       | Python 主后端， 负责事件循环、请求调度和状态管理                                                                     |
| python-lsp-bridge                   | 通过 `uv` 启动 Python 后端的 shell 启动脚本                                                                          |
| acm/                                | lsp-bridge 使用的异步补全菜单前端和补全界面                                                                          |
| core/fileaction.py                  | 每个文件的状态机， 负责路由请求、处理响应以及协调编辑器状态                                                         |
| core/lspserver.py                   | 每个语言服务器进程的 LSP 传输层， 负责请求和响应顺序控制                                                             |
| core/handler/                       | 各类 LSP 方法和编辑器动作的具体处理实现                                                                              |
| core/remote_file.py                 | 远程文件同步和远程编辑支持                                                                                           |
| core/utils.py, core/mergedeep.py    | 通用工具函数以及设置项、协议数据用到的深度合并工具                                                                   |
| core/search_*.py                    | 文件单词、路径和词典查询等异步搜索后端                                                                               |
| core/tabnine.py, core/codeium.py, core/copilot.py | AI 补全后端                                                                                           |
| langserver/                         | 单服务器 JSON 配置， 包含可执行命令、语言 ID、启动参数和设置项                                                       |
| multiserver/                        | 多服务器组合配置， 用于把补全、诊断等能力组合在一起                                                                  |
| resources/                          | 词典和其他辅助数据文件                                                                                               |
| test/                               | Python 和 Elisp 测试， 覆盖补全、跳转、字节编译等流程                                                                |
| pyproject.toml, uv.lock             | `uv` 使用的 Python 依赖和环境元数据                                                                                  |
| .github/                            | CI 工作流和仓库自动化配置                                                                                            |
