...
## Remote Usage
`lsp-bridge` can also provide code syntax completion for files on remote servers, just like VSCode does. For large and complex software with high resource requirements or complicated running environment configuration that is not convenient for local development, providing the function of remote code completion is very useful. The following are the steps to configure remote code completion:

1. Download [nova](https://github.com/manateelazycat/nova): `nova` is a multi-threaded remote file editing and synchronization plugin that uses multi-threading technology to ensure that editing remote files does not jam Emacs.
2. Copy `server.py` of `nova` to the remote server and execute the command `python3 server.py` to start the service of `nova` server.
3. Install the LSP Server, lsp-bridge and Emacs on the remote server.
4. Configure to load `lsp-bridge` plug-in by default in remote Emacs, and automatically start the `lsp-bridge` service `(lsp-bridge-start-process)`.
5. Use the command `nova-open-file` to open the remote file, enter the remote server IP and file path, such as `xxx.xxx.xxx.xxx:/path/file`.

After the remote file is opened, `lsp-bridge` will automatically pop up the completion menu. The principle of the cooperation between `lsp-bridge` and `nova` is:

1. Login to the remote server by `nova` through SSH authentication, and access and edit remote files.
2. When editing remote files locally, `lsp-bridge` will send the diff sequence to the `nova` server side in real time. The `nova` server will reconstruct the latest content of the file on the server side according to the diff sequence.
3. The LSP requests generated by editing remote files are forwarded to `lsp-bridge` on the server side. `lsp-bridge` will perform syntax completion calculation based on the latest content of the remote file and the LSP Server on the server side.
4. After `lsp-bridge` calculates the LSP completion menu items on the remote side, it sends the completion data to the local machine, and then the `lsp-bridge` on the local machine renders the completion menu.

If the completion menu does not pop up, please log in to the remote server, open Emacs, and check the contents of the `*lsp-bridge*` buffer. Generally, it is because the Emacs plug-in or LSP Server on the server side is not installed completely.
