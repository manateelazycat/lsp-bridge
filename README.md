...
| PowerShell  | [PowerShell Editor Services](https://github.com/PowerShell/PowerShellEditorServices)                | Install `powershell-mode`, `pwsh`, and a PowerShell Editor Services release. Set `LSP_BRIDGE_POWERSHELL_EDITOR_SERVICES_DIR` to the extracted release root, `LSP_BRIDGE_POWERSHELL_LOG_PATH` to a writable log file, and `LSP_BRIDGE_POWERSHELL_SESSION_DETAILS_PATH` to a writable session-details file. |
